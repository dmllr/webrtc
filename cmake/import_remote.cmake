...
function(git_config destdir command)
	string(REPLACE " " ";" argl ${command})
	list(GET argl 0 action)
	list(GET argl 1 key)
	list(GET argl 2 value)
	execute_process(
		COMMAND "${GIT_EXECUTABLE}" config --${action} ${key} ${value}
		WORKING_DIRECTORY "${destdir}"
		RESULT_VARIABLE exitstatus ERROR_VARIABLE err
		OUTPUT_QUIET
	)
	if (NOT "${exitstatus}" EQUAL 0)
		message(FATAL_ERROR "failed to configure git repository '${destdir}'\n${err}")
	endif()
endfunction()
...
function(git_do_all)
	...
	if (NOT EXISTS "${destdir}/.git")
		...
		if (OPTS_GIT_CONFIG)
			foreach(cnf ${OPTS_GIT_CONFIG})
				msg("configuring ${cnf}")

				git_config("${destdir}" "${cnf}")
			endforeach(cnf)
		endif()

		msg("updating ${project_id} [${branch}]")
		git_fetch("${destdir}")
